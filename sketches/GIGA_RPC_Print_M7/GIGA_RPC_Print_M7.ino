...
void loop() {
    ...
    int rx_avail = min(Serial.available(), sizeof(to_m4_buffer));
    int tx_avail = RPC.availableForWrite(); 
    if (rx_avail) {
        Serial.readBytes(to_m4_buffer, rx_avail);
        RPC.write(to_m4_buffer, rx_avail);
    }
}