...
class USBMTP: public internal::PluggableUSBModule{
public:
    ...
    */
    USBMTP(bool connect_blocking = true, const char* name = NULL, uint16_t vendor_id = 0x1f00, uint16_t product_id = 0x2012, uint16_t product_release = 0x0001);
    ...
    * @param product_release Your product_release
    */
    USBMTP(USBPhy *phy, const char* name, uint16_t vendor_id, uint16_t product_id, uint16_t product_release);
...
protected:
    ...
    void _change_mtp_connected(bool connected);
    ...
    usb_ep_t _bulk_out;
    usb_ep_t _int_in;
    enum {CONFIG1_DESC_SIZE = (9 + 9 + ENDPOINT_DESCRIPTOR_LENGTH + ENDPOINT_DESCRIPTOR_LENGTH + ENDPOINT_DESCRIPTOR_LENGTH)};

    uint8_t _config_descriptor[CONFIG1_DESC_SIZE];
    ...
    uint8_t _tx_buffer[MTP_MAX_PACKET_SIZE];
    uint8_t *_tx_buf;
    uint32_t _tx_size;

    OperationList<AsyncRead> _rx_list;
    bool _rx_in_progress;
    uint8_t _rx_buffer[MTP_MAX_PACKET_SIZE];
    uint8_t *_rx_buf;
    uint32_t _rx_size;
