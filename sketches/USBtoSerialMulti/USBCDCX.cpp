...
class USBCDCX::AsyncWrite : public AsyncOp {
  public:
    AsyncWrite(USBCDCX *serial, uint8_t *buf, uint32_t size)
...
class USBCDCX::AsyncRead : public AsyncOp {
  public:
    AsyncRead(USBCDCX *serial, uint8_t *buf, uint32_t size, uint32_t *size_read, bool read_all)
...
class USBCDCX::AsyncWait : public AsyncOp {
  public:
    AsyncWait(USBCDCX *serial)
        : serial(serial) {
    }
...
USBCDCX::USBCDCX(bool connect_blocking, const char *name, uint16_t vendor_id, uint16_t product_id, uint16_t product_release)
    : internal::PluggableUSBModule(2), extraDescriptor(name) {
    PluggableUSBD().plug(this);
}

USBCDCX::USBCDCX(USBPhy *phy, const char *name, uint16_t vendor_id, uint16_t product_id, uint16_t product_release)
    : internal::PluggableUSBModule(2), extraDescriptor(name) {
    PluggableUSBD().plug(this);
}
...
void USBCDCX::init(EndpointResolver &resolver) {
...
void USBCDCX::callback_state_change(USBDevice::DeviceState new_state) {
    assert_locked();
    /* Called in ISR context */
...
uint32_t USBCDCX::callback_request(const USBDevice::setup_packet_t *setup, USBDevice::RequestResult *result, uint8_t **data) {
    assert_locked();
    /* Called in ISR context */
...
bool USBCDCX::callback_request_xfer_done(const USBDevice::setup_packet_t *setup, bool aborted) {
    assert_locked();
    /* Called in ISR context */
...
bool USBCDCX::callback_set_configuration(uint8_t configuration) {
    assert_locked();
    /* Called in ISR context */
    ...
    PluggableUSBD().endpoint_add(_bulk_in, CDC_MAX_PACKET_SIZE, USB_EP_TYPE_BULK, ::mbed::callback(this, &USBCDCX::_send_isr));
    PluggableUSBD().endpoint_add(_bulk_out, CDC_MAX_PACKET_SIZE, USB_EP_TYPE_BULK, ::mbed::callback(this, &USBCDCX::_receive_isr));
...
void USBCDCX::callback_set_interface(uint16_t interface, uint8_t alternate) {
    assert_locked();
}

void USBCDCX::_change_terminal_connected(bool connected) {
    assert_locked();
...
bool USBCDCX::send(uint8_t *buffer, uint32_t size) {
    lock();
...
void USBCDCX::send_nb(uint8_t *buffer, uint32_t size, uint32_t *actual, bool now) {
    lock();
...
bool USBCDCX::receive(uint8_t *buffer, uint32_t size, uint32_t *size_read) {
    lock();
...
void USBCDCX::receive_nb(uint8_t *buffer, uint32_t size, uint32_t *size_read) {
...
void USBCDCX::_receive_isr_start() {
...
const uint8_t *USBCDCX::string_iinterface_desc() {
    return (const uint8_t *)extraDescriptor;
}

uint8_t USBCDCX::getProductVersion() {
    return 1;
}

const uint8_t *USBCDCX::string_iproduct_desc() {
...
const uint8_t *USBCDCX::configuration_desc(uint8_t index) {
