...
void loop()
{
  ...
  tft.fillWindow(ST77XX_RED);
  tft.setCursor(CENTER, CENTER);
  tft.print("Rotation: ");
  tft.print(test_screen_rotation);
...
uint32_t displayStuff1()
{
  ...
  tft.drawRect(rect_x, rect_y, 100, 40, ST77XX_WHITE);
  ...
  for (uint16_t y = rect_y + 5; y < rect_y + 40; y += 5)
    tft.drawFastHLine(rect_x + 1, y, 98, ST77XX_CYAN);
  for (uint16_t x = rect_x + 5; x < rect_x + 100; x += 5)
    tft.drawFastVLine(x, rect_y + 1, 38, ST77XX_CYAN);
  ...
  tft.drawRect(rect_x, rect_y, 100, 50, ST77XX_CYAN);
  ...
  for (uint8_t i = 0; i < (sizeof(alternating_text) - 1); i++) {
    if (i & 1) tft.setTextColor(ST77XX_WHITE, ST77XX_RED);
    else tft.setTextColor(ST77XX_YELLOW, ST77XX_BLUE);
