...
#ifndef _ST77XX_mbed_H_
#define _ST77XX_mbed_H_
...
class ST77XX_mbed : public Print {
public:
  // Constructor
  //   pspi: either  &SPI (6 pin spi connector) or &SPI1 (shield pins)
  //   CS: Chip select pin,  DC: Data/Command pin
  //   RST: optional reset pin
  ST77XX_mbed(SPIClass *pspi, uint8_t CS, uint8_t DC, uint8_t RST = 255);
  
  ST77XX_mbed(uint8_t cs_pin, uint8_t dc_pin, uint8_t rst_pin = 255);

  #ifdef ZEPHYR_PINNAMES_H
  // Allow Pin Names to be used if defined.
  ST77XX_mbed(SPIClass *pspi, PinName CS, PinName DC, PinName RST = NC);
  ST77XX_mbed(PinName cs_pin, PinName dc_pin, PinName PinName = NC);
  #endif
  ...
  static void spi_callback (const struct device *dev, int result, void *data) {
    ((ST77XX_mbed *)data)->process_spi_callback(dev, result);
  }
...
//  volatile uint8_t _data_sent_not_completed = 0;  // how much data has been sent that we are waiting for info
  ...
  static ST77XX_mbed *_dmaActiveDisplay[2]; // Use pointer to this as a way to get
                                         // back to object...
...
class ST7796_zephyr : public ST77XX_mbed  {
public:
...
class ST7789_zephyr : public ST77XX_mbed  {
public:
...
class ST7735_zephyr : public ST77XX_mbed  {
public:
